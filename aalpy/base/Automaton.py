...
class Automaton(ABC):
    ...
    def get_output_alphabet(self) -> list:
        ...
        """
        return list(set([s.output for s in self.states]))
    ...
    def visualize(self, path='LearnedModel', file_type='pdf', display_same_state_transitions=True, bOpenBrowser=False):
        from aalpy.utils import visualize_automaton
        visualize_automaton(self, path, file_type, display_same_state_transitions, bOpenBrowser=bOpenBrowser)
...
class DeterministicAutomaton(Automaton):
    ...
    def get_all_paths(self, origin_state: AutomatonState, target_state: AutomatonState, bShortestPath=False):
        all_inputs = []
                ...
                for neighbour in neighbours:
                    ...
                    if neighbour == target_state:
                        ...
                        if bShortestPath:
                            ...
                            all_inputs.append(tuple(inputs))
        ...
        if bShortestPath:
            ...
            all_paths = set([''.join(tup) for tup in all_inputs])
            suffix_paths = set()
            for s1 in all_paths:
                suffix_paths = suffix_paths.union(set([s for s in all_paths if s.startswith(s1) and s1 != s]))

            return all_paths.difference(suffix_paths)
    ...
    def get_shortest_path(self, origin_state: AutomatonState, target_state: AutomatonState) -> tuple:
        ...
        """
        return self.get_all_paths(origin_state=origin_state,target_state=target_state, bShortestPath=True)       
