...
class RPNI:
    ...
    def run_rpni(self, input_alphabet, bKernel):
        ...
        while blue:
            ...
            for red_state in red:
                if bKernel and (len(red_state.children)!=len(input_alphabet) or len(lex_min_blue.children)!=len(input_alphabet)):
                    continue
...
def run_RPNI(data, automaton_type, input_completeness=None, print_info=True, bKernel=False, input_alphabet=None) -> Union[DeterministicAutomaton, None]:
    """
    ...
    learned_model = rpni.run_rpni(bKernel=bKernel, input_alphabet=input_alphabet)
